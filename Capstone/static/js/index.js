...
function galleryLike(event) {
  // creating event or 'signal' in browser that will run the following code when prompted
  ...
  url = 'http://localhost:8000/gallerylike/' + galleryID;
  axios
    .post(url, {}, axios_config) // {} is representing the HTTP reuqest body
    .then((response) => {
      // once post request is made, Axios returns promise that is either fulfilled or rejected (response)
