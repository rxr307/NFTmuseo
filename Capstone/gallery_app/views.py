    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for dictionary in response:
                ...
                contract_address = 'https://api.opensea.io/api/v1/asset/' + \
                ...
                nft_created_date = datetime.strptime(
                    nft_created_date, '%Y-%m-%d').strftime('%m/%d/%Y')
...
def museumview(request):
    ...
    if request.method == 'GET':
        ...
        public_galleries = NewGallery.objects.filter(
            public_gallery=True).order_by('-created_date')
...
def filteredmuseum(request):
    ...
    if request.method == 'POST':
        ...
        filtered_galleries = NewGallery.objects.filter(
            public_gallery=True, gallery_category=category).order_by('-created_date')
